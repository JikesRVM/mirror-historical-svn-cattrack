...
class CreateResultFacts < ActiveRecord::Migration
  def self.up
    ...
    add_foreign_key :result_facts, [:host_id], :host_dimensions, [:id], :on_delete => :restrict
    add_foreign_key :result_facts, [:test_run_id], :test_run_dimensions, [:id], :on_delete => :restrict
    add_foreign_key :result_facts, [:test_configuration_id], :test_configuration_dimensions, [:id], :on_delete => :restrict
    add_foreign_key :result_facts, [:build_configuration_id], :build_configuration_dimensions, [:id], :on_delete => :restrict
    add_foreign_key :result_facts, [:build_target_id], :build_target_dimensions, [:id], :on_delete => :restrict
    add_foreign_key :result_facts, [:test_case_id], :test_case_dimensions, [:id], :on_delete => :restrict
    add_foreign_key :result_facts, [:time_id], :time_dimensions, [:id], :on_delete => :restrict
    add_foreign_key :result_facts, [:revision_id], :revision_dimensions, [:id], :on_delete => :restrict
    add_foreign_key :result_facts, [:result_id], :result_dimensions, [:id], :on_delete => :restrict
