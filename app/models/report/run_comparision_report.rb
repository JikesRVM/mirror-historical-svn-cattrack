...
SELECT
    ...
    max(case when build_configurations.test_run_id = #{@first_run.id} then test_cases.id else NULL end) AS test_case_id
...
SELECT * FROM (#{sql}) f
WHERE test_case_id IS NOT NULL AND first_run_success = 1 AND second_run_success != 1
...
SELECT * FROM (#{sql}) f
WHERE test_case_id IS NOT NULL AND in_first_run = 1 AND in_second_run != 1
ORDER BY build_configuration_name, test_configuration_name, group_name, test_case_name, test_case_execution_name
SQL
