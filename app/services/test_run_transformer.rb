...
class TestRunTransformer
      ...
      test_run.test_configurations.each do |tc|
        ...
        tc.groups.each do |g|
          g.test_cases.each do |t|
            create_test(host, build_target, revision, time, test_configuration, build_configuration, g.name, t)
          end
        end
      end
    end
  ...
  def self.create_test(host, build_target, revision, time, test_configuration, build_configuration, group_name, t)
